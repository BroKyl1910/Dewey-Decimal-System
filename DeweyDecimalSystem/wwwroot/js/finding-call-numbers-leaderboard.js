...
function initialDataCall() {
    $.ajax({
        method: "GET",
        url: "/Leaderboards/FindingCallNumbersLeaderboardData",
        success: (jsonString) => {
...
function buildLeaderboard(scoreViewModels) {
    ...
    for (var i = 0; i < scoreViewModels.length; i++) {
        var rowHtml = '<tr><th scope = "row" >' + (i + 1) + '</th><td>' + scoreViewModels[i].Name + '</td><td>' + scoreViewModels[i].Score + ' pts</td></tr >';
